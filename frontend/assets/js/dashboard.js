...
async function procesarPago() {
  ...
  try {
    await new Promise(r => setTimeout(r, 1600)); // simular verificación bancaria
...
async function verDetallePedido(id) {
  // Open modal immediately with cached data, then refresh with full data (images, producto_id)
...
function _renderDetallePedido(o) {
  ...
  const clienteEl = document.getElementById('dp-cliente-wrap');
  if (clienteEl) { clienteEl.style.display = o.cliente_nombre ? 'flex' : 'none'; set('dp-cliente', o.cliente_nombre || ''); }
  ...
  // Guardar pedido actual para recomprar
  ui._pedidoDetalle = o;
